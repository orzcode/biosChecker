...
export async function mailer(userObject, modelsObject) {
  ...
  const donator = Boolean(user.donator);

  const unsubLink = `${projectLink}unsubscribe?email=${encodeURIComponent(user.email)}`;

  const donatorLine = donator
    ? '<p><strong>💚 Thanks for being a donator! 💚  You have priority during busy updates.</strong></p>'
    : '';
...
<p>A new BIOS is available for your ASRock <strong>${models.mobo ?? user.mobo}</strong> motherboard.</p>
...
<ul>
<li>Ensure your USB drive is stable. Test in Windows by reading and writing to it - make sure it opens, reads, and writes without hanging.</li>
...
<li>Never power off during a BIOS update. If you absolutely have to (for example if it hangs for a very long time), try removing the USB instead.</li>
</ul>
<hr style="border: 1px solid #444;" />
...
export async function confirmationMail(userObject) {
  ...
  const unsubLink = `${projectLink}unsubscribe?email=${encodeURIComponent(user.email)}`;
  const confirmLink = `${projectLink}confirm/${encodeURIComponent(user.id)}`;
